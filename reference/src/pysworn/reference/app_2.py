...
import typer
from pysworn.reference.ruleset import RulesetTabs
...
class RulesetTabsApp(App[None]):
    ...
    #     msg = f"Selected: {id_}"
    #     self.query_one(Static).update(Pretty(msg))
    #     self.log(msg)
    ...
    #     ruleset_tabbed = self.query_one(RulesetTabbedContent)
    #     # with ruleset_tabbed.prevent(TabbedContent.TabActivated):
    #     with self.prevent(RulesetTabbedContent.RulesetChanged):
    #         with self.prevent(CategoryTabbedContent.CategoryChanged):
    #             with self.prevent(ReferenceTree.ReferenceHighlighted):
    #                 ruleset_tabbed.current_id = id_

    # @on(RulesetTabbedContent.RulesetChanged)
    # def on_ruleset_changed(self, event: RulesetTabbedContent.RulesetChanged) -> None:
    #     event.stop()
    #     self.log(f"app: Ruleset changed to: {event.ruleset}")
    ...
    #     event.stop()
    #     self.log(f"app: Category changed to: {event.category}")
    ...
    #     event.stop()
    #     self.log(f"app: Node changed to: {event.id_}")
...
def main(
    ...
    inline: Annotated[
        bool,
        typer.Option("--inline", "-i", help="Run the app inline in the terminal."),
    ] = False,
) -> None:
    ...
    if log_level == "DEBUG":
        from .logging import print_tree

        print_tree()

    app = RulesetTabsApp()
    app.log(f"inline: {inline}")
